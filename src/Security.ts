...
export class Security {
  ...
  // private member vars
  private static authedUsers = new Array<IUser>();
  private static expirationTimer?: NodeJS.Timeout;
   ...
   * @return {USER_ROLE}
   */
  public getUserCreds(userName: string): IUser | null {
   ...
   * @param minRole
   */
  public userHasRole(authHeader: string | undefined, minRole: USER_ROLES): boolean {
    if (authHeader !== undefined) {
      const fields = authHeader.split(' ');
      if (fields.length > 0) {
        const b64Creds: string | undefined = fields.pop();
        if (b64Creds !== undefined && b64Creds.length > 0) {
          const creds = Buffer.alloc(b64Creds.length, b64Creds, 'base64')
            .toString('ascii')
            .split(':');
          const user: IUser | null = this.getUserCreds(creds[0]);
          if (user !== null) {
   ...
   * @param authedUser
   */
  public cacheAuthedUser(authedUser: IUser) {
  ...
  private expireCredentials() {
    ...
    const now = Date.now();
    const validUsers = new Array<IUser>();
